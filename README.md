...
- [ ] **_find all customers that include the word 'MARKET' in the contact title.
  ...
  - Don't forget the wildcard '%' symbols at the beginning and end of your
    substring to denote it can appear anywhere in the string in question
  - Remember to convert your contact title to all upper case for case
  insensitive comparing so upper(contact_title)
  </details>
...
- [ ] **_list orders grouped and ordered by customer company name showing the
  ...
  - This can be done with SELECT, COUNT, JOIN and GROUP BY clauses. Your count
    should focus on a field in the Orders table, not the Customer table
  - There is more information about the COUNT clause on
  [W3 Schools](https://www.w3schools.com/sql/sql_count_avg_sum.asp)
  </details>
...
- [ ] **_list customers by contact name and the number of orders per contact
  ...
  - This can be done by adding an ORDER BY clause to the previous answer and
...
- [ ] **_list orders grouped by customer's city showing the number of orders per
  ...
  - This is very similar to the previous two queries, however, it focuses on the
  City rather than the Customer Names
  </details>
